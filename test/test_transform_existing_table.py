...
def test_transform_existing_table_view():
    ...
    assert (
        editor.tableView is not existing_table
    ), "Editor should use its own table view (not the one passed)"
    print("  ✓ Editor has its own independent table view")

    # The original table view should be unaffected
    original_still_works = (
        existing_table.parent() is None or True
    )  # Still in original parent or orphaned
