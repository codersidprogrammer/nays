...
class DataTableHandler(QObject):
    ...
    def getColumnVectorAsNumpy(self, column: int):
        ...
        return np.array(
            [
                float(self.table.item(row, column).text())
    ...
    def loadDataFromNumpy(
        self, data: np.ndarray | list, is_override: bool = True, is_formatted: bool = False
    ):
        """
    ...
    def loadDataFromNumpyThenChange(
        self, data: np.ndarray, allowEmit: bool = True, isFormatted: bool = False
    ):
...
class DataTableBuilderHandler(DataTableHandler):
    ...
    def getValuesAsDict(self, getKeyFromHeaderText: bool = True) -> dict:
        ...
        for row in range(self.table.rowCount()):
            ...
            row_name = (
                header_item.text()
                if getKeyFromHeaderText
                else header_item.data(Qt.ItemDataRole.WhatsThisPropertyRole)
            )
    ...
    def getValueAsModel(
        self, getKeyFromHeaderText: bool = True
    ) -> list[list[TableHandlerDataModel]]:
        """
        ...
        for row in range(self.table.rowCount()):
            ...
            row_header_id = (
                row_header_item.data(Qt.ItemDataRole.WhatsThisPropertyRole)
                if row_header_item
                else None
            )
                            ...
                            if checkbox:
                                values.insert(
                                    ...
                                    TableHandlerDataModel(
                                        ...
                                        items=[
                                            {i: widget.itemText(i)} for i in range(widget.count())
                                        ],
                                    ),
                                )
    ...
    def setValueFromModelThenChange(
        self,
        models: list[list[TableHandlerDataModel]],
        allowEmit: bool = True,
        rebuildHeaders: bool = True,
    ):
        oldValue = self.allowEmitDataChange
        self.allowEmitDataChange = allowEmit
    ...
    def setValueFromModelAsColumnsThenChange(
        self,
        models: list[list[TableHandlerDataModel]],
        allowEmit: bool = True,
        rebuildHeaders: bool = True,
    ...
    def setValueFromModel(
        self, models: list[list[TableHandlerDataModel]], rebuildHeaders: bool = True
    ):
        """
            ...
            for col_idx, model in enumerate(row_models):
                ...
                match model.type:
                    case "combobox":
                        ...
                        if (
                            isinstance(model.defaultValueIndex, int)
                            and model.defaultValueIndex < combo.count()
                        ):
                    ...
                    case "checkbox":
                        ...
                        # Set checked state
                        is_checked = (
                            bool(model.defaultValueIndex)
                            if model.defaultValueIndex is not None
                            else False
                        )
    ...
    def setValueFromModelAsColumns(
        self, models: list[list[TableHandlerDataModel]], rebuildHeaders: bool = True
    ):
        """
        ...
        row_count = (
            len(models[0]) if models[0] else 0
        )  # Number of rows = length of first inner list
    ...
    def comboCallback(self, row: int, combo: QComboBox, param: str, index: int = None):
        ...
        # get from latest
        models = (
            self.getValueAsModelFromRows()
            if self.tableType == "columns"
            else self.getValueAsModel()
        )
        rowModel = models[row]
        ...
        for i, colDict in enumerate(wamitconf):
            ...
            match col_type:
                ...
                case "checkbox":
                    ...
                    checkbox.stateChanged.connect(
                        partial(
                            self.checkboxCallback, 0, checkbox_label, col_name
                        )  # row 0 for horizontal
                    )
    ...
    def setTableRowsForColumnsThenChange(
        self, rowCount: int, defaultValue=None, allowEmit: bool = True
    ):
        oldValue = self.allowEmitDataChange
        self.allowEmitDataChange = allowEmit
        ...
        if rowCount < currentRowCount or rowCount == 0:
            ...
            for row in range(currentRowCount, rowCount):
                for col in range(columnCount):
                    ...
                    col_type = header_item.data(Qt.ItemDataRole.UserRole)
                    col_name = (
                        header_item.data(Qt.ItemDataRole.WhatsThisPropertyRole)
                        or header_item.text()
                    )
                            ...
                            if source_widget is None:
                                ...
                                if self.data:
                                    for colDict in self.data:
                                        if (
                                            colDict.get("name") == col_name
                                            and colDict.get("type") == "combobox"
                                        ):
                                            ...
                                            combo.setCurrentIndex(
                                                int(colDict.get("defaultValueIndex", 0))
                                            )
                                            break
                            ...
                            if source_container is None:
                                ...
                                if self.data:
                                    for colDict in self.data:
                                        if (
                                            colDict.get("name") == col_name
                                            and colDict.get("type") == "checkbox"
                                        ):
                                            checkbox.setChecked(
                                                bool(colDict.get("defaultValueIndex", False))
                                            )
                                            break
                                else:
                                    checkbox.setChecked(False)
                                ...
                                if source_checkbox:
                                    ...
                                    checkbox.stateChanged.connect(
                                        partial(
                                            self.checkboxCallback, row, checkbox_label, col_name
                                        )
                                    )
    ...
    def setTableColumnsForRowsThenChange(
        self, colCount: int, defaultValue=None, allowEmit: bool = True
    ):
        oldValue = self.allowEmitDataChange
        self.allowEmitDataChange = allowEmit
            ...
            for col in range(currentColumnCount, columnCount):
                for row in range(rowCount):
                    ...
                    if source_header_item:
                        ...
                        row_name = (
                            source_header_item.data(Qt.ItemDataRole.WhatsThisPropertyRole)
                            or source_header_item.text()
                        )
                                ...
                                if source_widget is None:
                                    ...
                                    if self.data:
                                        for rowDict in self.data:
                                            if (
                                                rowDict.get("name") == row_name
                                                and rowDict.get("type") == "combobox"
                                            ):
                                                # Add items from configuration
                                                for item in rowDict.get("items", []):
                                                    for key, strValue in item.items():
                                                ...
                                                if (
                                                    isinstance(row_default_value, int)
                                                    and 0 <= row_default_value < combo.count()
                                                ):
                                                    combo.setCurrentIndex(row_default_value)
                                                else:
                                                    combo.setCurrentIndex(
                                                        int(rowDict.get("defaultValueIndex", 0))
                                                    )
                                                break
                                    ...
                                    if isinstance(row_default_value, bool):
                                        ...
                                        if self.data:
                                            for rowDict in self.data:
                                                if (
                                                    rowDict.get("name") == row_name
                                                    and rowDict.get("type") == "checkbox"
                                                ):
                                                    checkbox_state = bool(
                                                        rowDict.get("defaultValueIndex", False)
                                                    )
                                                    break
    ...
    def loadDataFromArray2D(
        self, data_array: list, column_headers: list[str] = None, allowEmit: bool = True
    ):
        oldValue = self.allowEmitDataChange
        self.allowEmitDataChange = allowEmit
        ...
        if column_headers is None:
            ...
            if len(column_headers) < column_count:
                column_headers.extend(
                    [f"Column {i+1}" for i in range(len(column_headers), column_count)]
                )
            else:
...
class DataTableBuilder(DataTableHandler):
    ...
    def loadDataFromNumpy(self, data, is_formatted: bool = False):
        ...
        if rows != self.table.rowCount():
            raise ValueError(
                f"Data row count ({rows}) does not match table row count ({self.table.rowCount()})"
            )
