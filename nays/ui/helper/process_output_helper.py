        ...
        try:
            if self.wamit_process is not None:
                ...
                result = self.wamit_process.executeWamit(
                    wamitExePath=wamit_exe_path, workingDir=working_dir
                )
...
class ProcessOutputWidget(QWidget):
    ...
    def runWamitProcess(
        self, wamit_capture: WamitProcessOutputCapture, wamit_exe_path: str, working_dir: str
    ):
    ...
    def onProcessFinished(self, exit_code: int):
        ...
        if exit_code == 0:
            ...
            self.appendError(
                f"Process failed with exit code {exit_code} after {runtime:.1f} seconds"
            )
        ...
        if filename:
            try:
                with open(filename, "w", encoding="utf-8") as f:
                    f.write(
                        f"Process Output Export - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                    )
                    f.write("=" * 60 + "\n\n")
                    f.write("MAIN OUTPUT:\n")
...
def createProcessOutputSubwindow(
    mdiArea, title: str, process_function: Callable = None, *args, **kwargs
):
    """
...
def createCharm3DProcessOutputSubwindow(
    mdiArea, title: str, charm3d_process, working_dir: str
) -> tuple[QMdiSubWindow, ProcessOutputWidget]:
    """
...
def createBasePySideExecutorOutputSubwindow(
    mdiArea, title: str, executor
) -> tuple[QMdiSubWindow, ProcessOutputWidget]:
    """
...
def createWamitProcessOutputSubwindow(
    mdiArea, title: str, wamit_process, wamit_exe_path: str, working_dir: str
) -> tuple[QMdiSubWindow, ProcessOutputWidget]:
    """
