...
class NaysModuleBase(ABC):
    ...
    def getMetadata(cls) -> ModuleMetadata:
        ...
        return ModuleMetadata(
            providers=cls.providers, imports=cls.imports, exports=cls.exports, routes=cls.routes
        )
            ...
            if provider.inject:
                ...
                def factory_with_deps(**kwargs):
                    ...
                    for dep in provider.inject:
                        dependencies[
                            dep.__name__.lower() if hasattr(dep, "__name__") else str(dep)
                        ] = self.injector.get(dep)
                    return (
                        provider.useFactory(**dependencies)
                        if dependencies
                        else provider.useFactory()
                    )
