...
class Router:
    ...
    def navigate(self, path: str, data: dict = {}):
        ...
        if self.__currentInstance is not None:
            ...
            if issubclass(type(self.__currentInstance), OnDestroy) and callable(
        ...
        if (
            issubclass(type(routeInstance), OnInit) or hasattr(routeInstance, "onInit")
        ) and callable(getattr(routeInstance, "onInit", None)):
    ...
    def back(self):
        ...
        if self.__currentInstance is not None:
            if hasattr(self.__currentInstance, "onDestroy") and callable(
                self.__currentInstance.onDestroy
            ):
                self.__currentInstance.onDestroy()
        ...
        if not self.__routes:
            ...
            for path, route in sorted(self.__routes.items()):
                route_type = (
                    ...
                    if hasattr(route.routeType, "name")
                    else str(route.routeType)
                )
                component_name = (
                    route.component.__name__
                    if hasattr(route.component, "__name__")
                    else str(route.component)
                )
    ...
    def getRoutesByType(self, route_type) -> Dict[str, Route]:
        ...
        return {
            path: route for path, route in self.__routes.items() if route.routeType == route_type
        }
